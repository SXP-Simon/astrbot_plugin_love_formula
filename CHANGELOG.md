...
## [v1.0.13] - `/学习` 命令解决冷启动问题，并发消息处理优化，并发 LLM 调用优化，按群隔离消息

*   **feat**: `/学习` 命令：指定回复一条消息，让插件记录该消息往后的所有消息，避免刚刚安装插件没有数据的冷启动问题。
*   **feat**: 按群隔离消息，避免消息混乱。（！！！注意这次更新请先删除插件持久化数据）
*   **feat**: 并发消息处理优化，并发 LLM 调用优化。
